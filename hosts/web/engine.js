...
function safeFrame() {
  ...
  try {
    ...
    // arg5: drained touch event stream (one packed u32 per sample). The
    // snapshot/hit-fact tracks remain available as args 3/4.
    const events = touchSamples.length > 0
      ? touchSamples.map((s) => (((s.id & 0xf) << 28) | ((s.dt & 0x3f) << 22) | ((s.y & 0x3ff) << 12) | ((s.x & 0x3ff) << 2) | (s.phase & 0x3)) >>> 0)
      : undefined;
    touchSamples = [];
    frameCb(held, packedAnalog(), undefined, undefined, events);
